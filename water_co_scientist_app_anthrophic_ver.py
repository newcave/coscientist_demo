...
# ──────────────────────────────────────────
# HELPER: Anthropic API 호출
# ──────────────────────────────────────────
def call_agent(api_key: str, mode_key: str, messages: list, pdf_b64: str = None):
    client = anthropic.Anthropic(api_key=api_key)
    system_prompt = next(v["system"] for k, v in AGENT_MODES.items() if v["key"] == mode_key)

    # ── 첫 번째 user 메시지 인덱스 탐색 ──
    first_user_idx = next(
    ...
    for idx, msg in enumerate(messages):
        if msg["role"] == "user":
            ...
            # PDF는 첫 번째 user 메시지에만 한 번 첨부
            if pdf_b64 and idx == first_user_idx:
                content.append({
                    "type": "document",
                    "source": {
                        "type": "base64",
    ...
    # content 블록 중 text 타입만 추출해서 합치기
    answer = "".join(
...
with st.sidebar:
    ...
    # ── API KEY ──
    st.markdown('<div class="sb-title">🔑 Anthropic API Key</div>', unsafe_allow_html=True)
    ...
    if uploaded_pdf is not None:
        if st.session_state.pdf_name != uploaded_pdf.name:
            with st.spinner("PDF 분석 중..."):
                st.session_state.pdf_content = pdf_to_base64(uploaded_pdf)
...
if not st.session_state.api_key:
    st.markdown("""
    <div class="warn-box">
        ⚠️ <strong>Anthropic API Key가 필요합니다.</strong><br>
    ...
    with st.spinner("🔄 Water Co-Scientist 분석 중..."):
        try:
            answer, tokens = call_agent(
                ...
                pdf_b64=st.session_state.pdf_content,
            )
            st.session_state.messages.append({"role": "assistant", "content": answer})
            st.session_state.total_tokens += tokens
            st.rerun()
        except anthropic.AuthenticationError:
            st.error("❌ API Key가 유효하지 않습니다. 사이드바에서 올바른 Anthropic API Key를 입력하세요.")
...
<div style="display:flex; justify-content:space-between; align-items:center;
    ...
    <div>
        <span class="tool-chip">claude-sonnet-4</span>
